...
void readplane_fix_signalpos() {

  TChain *tIn = new TChain("TofHitsTree");
  {
    tIn->Add(TString::Format(
        "/home/qpham/IProject/tofreco/run16573_outputTree.root"));
  }
  ...
  Int_t SignalBar, SignalPlane, SignalType, HitFeb, HitSampicChannel, HitSampic,
      HitDaqChannel, HitPlane, HitBar, HitEdge, SignalHitsListIndex;

  Double_t SignalPosition;

  std::vector<double> *SignalPosition_T, *SignalPosition_B;

  Double_t SignalTime, HitCell0Time, HitTotValue, HitBaseline, HitRawPeak,
      HitPeak, HitPeakTime, HitCfTime_0p1, HitCfTime_0p9, HitVoltageIntegral,
      HitRawTotValue, HitUnixTime, HitSampleLength, HitPeakSample,
      HitChannelOnPlane, HitSignalIndex, *HitIsSaturated, HitFitSuccess,
      HitCfSuccess, HitWaveform, HitFitParameter;
  ...
  tIn->SetBranchAddress("TofHitsTree.SignalHitsListIndex",
                        &SignalHitsListIndex);
  ...
  TH2D *hist_top_bot = new TH2D("hist_top_bot", "Top vs Bottom Plane", 100, -50,
                                50, 100, -50, 50);
  TH2D *hist_top_upstream = new TH2D("hist_top_upstream", "Top vs Upstream",
                                     100, -50, 50, 100, -50, 50);
  TH2D *hist_top_downstream = new TH2D(
      "hist_top_downstream", "Top vs Downstream", 100, -50, 50, 100, -50, 50);
  ...
  for (Long64_t iEntry = 1; iEntry < nEntries; ++iEntry) {
    ...
    if (SignalPlane == 3) { // Top Plane
      SignalPosition_T->at(iEntry) = SignalPosition;
      //    if (SignalPosition != -9999) { // Some condition to avoid invalid
      //    values
      hist_top_bot->Fill(SignalPosition_T->at(iEntry),
                         SignalPosition_T->at(iEntry));
  ...
  // Draw histograms
  TCanvas *top_vs_planes =
      new TCanvas("top_vs_plane", "Correlations", 800, 600);
