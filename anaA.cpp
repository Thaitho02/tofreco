...
int main(int argc, char *argv[]) {
    ...
    using namespace std;
    string filename = "SignalHit.txt";
    ofstream outFile(filename);

    // Kiểm tra xem file có mở thành công không
    ifstream checkFile(filename);
    if (!checkFile) {
        cerr << "Error: File was not created or cannot be opened: " << filename << endl;
     
    } else {
        cout << "File created and data written successfully: " << filename << endl;
    }
...
//    clp.getDescription() << "> and performs a primary event reconstruction, mainly for monitoring." << std::endl;
    ...
    // time Asymmetry
    TH1F *hTimeAsymmetry = new TH1F("hTimeAsymmetry", "Time Asymmetry Histogram;Time Asymmetry (s);Counts", 30, -0.1, 0.1);
        ...
        for (auto it1 = signals.begin(); it1 != signals.end(); ++it1) { // Loop over the signals
            for (auto it2 = std::next(it1); it2 != signals.end(); ++it2) {
                ...
                if (signal1.GetSignalType() == 3 && signal2.GetSignalType() == 3) {
                    ...
                    getCoordinate(signal1.GetSignalPlane(), signal1.GetSignalBar(), signal1.GetSignalPosition(), x1, y1, z1);
                    getCoordinate(signal2.GetSignalPlane(), signal2.GetSignalBar(), signal2.GetSignalPosition(), x2, y2, z2);
                    // quang duong bayl
                    ...
                    // thơi gian bay ta xác địn từ vị trí 
                    double t_a = (1/(3*10^{8}))*sqrt(pow(x2 - x1, 2) + pow(y2 - y1, 2) + pow(z2 - z1, 2));
                    // bất đối xứng
                    double timeAsymmetry =  (t_a - timeDiff )/(t_a + timeDiff)
                    ...
                    if(signal1.GetSignalTime() > signal2.GetSignalTime()){
                        ...
                        //outFile << x1 << " " << y1 << " " << z1 << " " << signal1.GetSignalPlane() << " " << signal1.GetSignalBar() << " " << signal1.GetSignalPosition() << " " << signal1.GetSignalTime() << endl;
                        cout << " Other Track : start and end " << endl ;
                        cout <<  thisTrack.start.x << " " << thisTrack.start.y  << " " << thisTrack.start.z << " " <<  thisTrack.start.plane << " " <<  thisTrack.start.bar << " " << thisTrack.start.position  << " " <<  thisTrack.start.time << "  " <<  thisTrack.timeDiff << endl;
                        cout <<  thisTrack.end.x << " " << thisTrack.end.y  << " " << thisTrack.end.z << " " <<  thisTrack.end.plane << " " <<  thisTrack.end.bar << " " << thisTrack.end.position  << " " <<  thisTrack.end.time << endl;
...
// cm/ns
                    ...
                    // Fill histogram TimeAsymmetry
                    hTimeAsymmetry->Fill(timeAsymmetry);
                    tree->Fill();
//                    tracks.push_back(thisTrack);
    ...
    // Draw histogram
    TCanvas *canvas = new TCanvas("canvas", "Time Asymmetry", 800, 600);
    hTimeAsymmetry->Draw();

    // Show the canvas
    canvas->Update();
    canvas->SaveAs("TimeAsymmetryHistogram.png");
